...
class DataTransformation:
    ...
    def get_data_transformation_object(self):
        try:
            ...
            # Define columns which should be categorical and numerical

            categorical_cols = ['Weather_conditions', 'Road_traffic_density', 'Type_of_order', 'Type_of_vehicle', 'Festival', 'City']
            numerical_cols = ['Delivery_person_Age', 'Delivery_person_Ratings', 'Vehicle_condition', 'multiple_deliveries', 'distance', 'Day', 'Month', 'Year']
    ...
    def initiate_data_transformation(self,train_path,test_path):
        try:
            ...
            logging.info('read train and test test Data completed')
            ...
            target_Delv_Time_Name = 'Time_taken (min)'
            #Drop if any unwanted columns like ID, Target etc
            drop_columns = [target_Delv_Time_Name]

            input_feature_train_df = train_df.drop(columns=drop_columns, axis=1)
            target_feature_train_df = train_df[target_Delv_Time_Name]
            
            input_feature_test_df = test_df.drop(columns=drop_columns, axis=1)
            target_feature_test_df = test_df[target_Delv_Time_Name]

            # Tranforming using preprocesser obj
